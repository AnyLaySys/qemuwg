...
public sealed partial class QEMU服务
{
    ...
    public Task<QEMU安装> DetectAsync() => Task.Run(async () =>
    {
        ...
        var versionResult = await 进程.RunAsync(primary.ExecutablePath, ["--version"]);
    ...
    public Task<QEMU能力> GetCapabilitiesAsync(QEMU架构 arch) =>
        capabilityCache.GetOrAdd(arch.Id, _ => QueryCapabilitiesAsync(arch));

    public Task<IReadOnlyList<QEMU设备属性>> GetDevicePropertiesAsync(QEMU架构 arch, string device) =>
        devicePropertyCache.GetOrAdd($"{arch.Id}:{device}", _ => QueryDevicePropertiesAsync(arch, device));

    public string? FindFirmware(QEMU安装 install, string arch, bool variables)
    {
    ...
    private static async Task<QEMU能力> QueryCapabilitiesAsync(QEMU架构 arch)
    {
        var machineTask = 进程.RunAsync(arch.ExecutablePath, ["-machine", "help"]);
        var cpuTask = 进程.RunAsync(arch.ExecutablePath, ["-cpu", "help"]);
        var accelTask = 进程.RunAsync(arch.ExecutablePath, ["-accel", "help"]);
        var displayTask = 进程.RunAsync(arch.ExecutablePath, ["-display", "help"]);
        var deviceTask = 进程.RunAsync(arch.ExecutablePath, ["-device", "help"]);
        var helpTask = 进程.RunAsync(arch.ExecutablePath, ["-help"]);
    ...
    private static async Task<IReadOnlyList<QEMU设备属性>> QueryDevicePropertiesAsync(QEMU架构 arch, string device)
    {
        var result = await 进程.RunAsync(arch.ExecutablePath, ["-device", $"{device},help"]);
    ...
    private static IReadOnlyList<QEMU命令选项> ParseCmdOptions(string output)
    {
