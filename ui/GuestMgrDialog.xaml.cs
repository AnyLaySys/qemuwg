...
public sealed partial class GuestMgrDialog : ContentDialog
{
    ...
    private readonly List<GuestAgentCmdInfo> commands = [];
    private bool confirmationPending;

    public GuestMgrDialog(nint ownerHandle, QemuSessionMgr sessions, VmCfg machine)
    {
        InitializeComponent();
    ...
    public ObservableCollection<GuestAgentCmdInfo> VisibleCommands { get; } = [];
    ...
    private static IEnumerable<GuestAgentCmdInfo> ParseCommands(string json)
    {
        using var document = JsonDocument.Parse(json);
        if (!document.RootElement.TryGetProperty("supported_commands", out var list)) yield break;
        foreach (var item in list.EnumerateArray())
            yield return new GuestAgentCmdInfo
            {
    ...
    private Task<GuestAgentResult> QueryIfSupportedAsync(string command) => Supports(command)
        ? sessions.ExecuteGuestAgentAsync(machine, command)
        : Task.FromResult(new GuestAgentResult(false, string.Empty, "Unsupported"));
    ...
    private async void RunProcess_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                ["path"] = path,
                ["arg"] = new JsonArray(CmdLineParser.Split(ProcessArgumentsBox.Text).Select(value => (JsonNode)value).ToArray()),
            ...
            for (var attempt = 0; attempt < 600; attempt++)
            {
                ...
                ProcessOutputBox.Text = FormatProcessResult(statusDocument.RootElement);
                return;
            }
    ...
    private static string FormatProcessResult(JsonElement root)
    {
        var lines = new List<string>();
    ...
    private void CommandList_SelectionChanged(object sender, SelectionChangedEventArgs e) { if (CommandList.SelectedItem is GuestAgentCmdInfo command) CommandBox.Text = command.Name; ResetConfirmation(); }
