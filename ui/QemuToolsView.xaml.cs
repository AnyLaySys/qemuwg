...
public sealed partial class QemuToolsView : UserControl
{
    ...
    private readonly QemuToolSessionMgr sessions;
    private readonly QemuToolSvc tools = new();
    private readonly List<QemuIoCmdInfo> allCommands = [];
    ...
    public QemuToolsView(nint ownerHandle, QemuInstall install, QemuToolSessionMgr sessions)
    {
        InitializeComponent();
    ...
    private async void InitializeOnFirstLoad(object sender, RoutedEventArgs args)
    {
        ...
        catch (Exception exception) { AppLog.Write("QemuToolsView initialization failed: " + exception); }
    }

    public ObservableCollection<QemuIoCmdInfo> VisibleCommands { get; } = [];
    ...
    {
        if (IoCommandList.SelectedItem is QemuIoCmdInfo command) IoCommandBox.Text = command.Name;
        ResetConfirmation();
    }
    ...
    private void StartNbd_Click(object sender, RoutedEventArgs e)
    {
        ...
        arguments.AddRange(CmdLineParser.Split(NbdExtraBox.Text));
    ...
    private static string JoinArguments(IEnumerable<string> arguments) => string.Join(' ', arguments.Select(CmdLineParser.Quote));
    private void AppendNbdResult(OperationResult result) => NbdOutputBox.Text += (NbdOutputBox.Text.Length == 0 ? string.Empty : Environment.NewLine) + ResultText(result);
    private void AppendStorageResult(OperationResult result) => StorageOutputBox.Text += (StorageOutputBox.Text.Length == 0 ? string.Empty : Environment.NewLine) + ResultText(result);
    private static string ResultText(OperationResult result) => result.Succeeded ? result.Message : string.Join(Environment.NewLine, new[] { result.Message, result.Detail }.Where(value => !string.IsNullOrWhiteSpace(value)));
    ...
    private void Sessions_OutputReceived(object? sender, ToolOutputEvent e) => DispatcherQueue.TryEnqueue(() =>
    {
    ...
    private void AppendResult(OperationResult result) => AppendOutput(result.Succeeded ? result.Message : string.Join(Environment.NewLine, new[] { result.Message, result.Detail }.Where(value => !string.IsNullOrWhiteSpace(value))));
    ...
    private static string FormatResult(ProcessResult result) => string.Format(T("tools.exitCode", "退出码 {0}"), result.ExitCode) + Environment.NewLine + (string.IsNullOrWhiteSpace(result.Output) ? T("tools.noOutput", "（无输出）") : result.Output);
}
