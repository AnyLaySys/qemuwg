...
public sealed partial class QemuSessionMgr
{
    ...
    public QemuSessionMgr(QemuSvc qemuSvc)
    {
        this.qemuSvc = qemuSvc;
    }

    public event EventHandler<VmCfg>? StateChanged;

    public OperationResult Start(QemuInstall install, VmCfg vm)
    {
        lock (gate)
        {
            if (sessions.TryGetValue(vm.Id, out var existing) && !existing.Process.HasExited)
                return OperationResult.Fail(T("session.alreadyRunning", "虚拟机已在运行"));
        }

        var arch = install.Archs.FirstOrDefault(item => item.Id == vm.Arch);
        if (arch is null) return OperationResult.Fail(T("session.unsupportedArchitecture", "当前 QEMU 不支持该系统架构"));
        if (!File.Exists(vm.DiskPath)) return OperationResult.Fail(T("session.diskMissing", "找不到虚拟磁盘"), vm.DiskPath);

        try
        {
            var port = FindFreePort();
            int guestAgentPort;
            do guestAgentPort = FindFreePort(); while (guestAgentPort == port);
        ...
        {
            return OperationResult.Fail(T("session.startFailed", "QEMU 启动失败"), exception.Message);
        }
    }

    public async Task<OperationResult> ShutdownAsync(VmCfg vm)
    {
        var result = await ExecuteQmpAsync(vm, "system_powerdown");
        return result.Succeeded
            ? OperationResult.Ok(T("session.shutdownRequested", "已发送关机请求"))
    ...
    public OperationResult ForceStop(VmCfg vm)
    {
        ...
        try
        {
            ...
            return OperationResult.Ok(T("session.forceStopped", "虚拟机已强制停止"));
        }
        catch (Exception exception)
        {
            return OperationResult.Fail(T("session.stopFailed", "无法停止虚拟机"), exception.Message);
        }
    }

    private IEnumerable<string> BuildArgs(QemuInstall install, VmCfg vm, int qmpPort, int guestAgentPort)
    {
        ...
        arguments.AddRange(["-display", string.IsNullOrWhiteSpace(vm.DisplayBackend) ? "gtk" : vm.DisplayBackend]);
