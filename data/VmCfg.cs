...
public sealed class VmCfg : INotifyPropertyChanged
{
    ...
    public List<QemuOptionEntry> QemuOpts { get; set; } = [];
    public List<QemuDeviceEntry> Devices { get; set; } = [];
    ...
    public string StatusText => IsRunning
        ? LocaleSvc.Current.Get("status.running", "运行中")
        : LocaleSvc.Current.Get("status.off", "已关机");

    public VmCfg Copy() => new()
    {
        Id = Id,
        Name = Name,
        DirPath = DirPath,
        ...
        QemuOpts = QemuOpts.Select(option => new QemuOptionEntry { Name = option.Name, Value = option.Value }).ToList(),
        Devices = Devices.Select(device => new QemuDeviceEntry
        {
            Model = device.Model,
