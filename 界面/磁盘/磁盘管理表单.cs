...
public sealed partial class 磁盘管理
{
    private void BuildForm(磁盘命令 command)
    {
        inputs.Clear();
    ...
    private FrameworkElement CreateField(磁盘字段 field)
    {
        if (field.Mode == 磁盘字段模式.Flag)
        {
    ...
    private string DefaultValue(磁盘字段 field)
    {
    ...
    private string ChoiceLabel(磁盘字段 field, string value)
    {
    ...
    private async Task BrowsePathAsync(磁盘字段 field, TextBox textBox)
    {
        if (field.PathKind == 磁盘路径类型.Save)
        {
        ...
        if (field.Mode == 磁盘字段模式.MultiPositional)
        {
